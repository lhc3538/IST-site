...
ShowDisciplines.Source = "SELECT * FROM keyicms_News WHERE SortID = " + Replace(ShowDisciplines__MMColParam, "'", "''") + " ORDER BY AddTime DESC"
...
ShowList.Source = "SELECT AddTime, bold, color, ID, NewsName, SortID, ViewFlag FROM keyicms_News WHERE SortID = " + Replace(ShowList__MMColParam, "'", "''") + " ORDER BY AddTime DESC"
                          ...
                          <% Dim NextTitle
						   While ((Repeat2__numRows <> 0) AND (NOT ShowSort.EOF)) 
								...
								if (ShowSort.Fields.Item("ViewFlag").Value) = -1 then							%>
                          			<li><a href="/ShowNewsList.asp?SortID=<%=(ShowSort.Fields.Item("ID").Value)%>&PageNum=1"><%=(ShowSort.Fields.Item("SortName").Value)%></a></li>
                            <% 	end if
						...
						<ul class="list">
                          <% While ((Repeat1__numRows <> 0) AND (NOT ShowDisciplines.EOF)) 
								if (ShowDisciplines.Fields.Item("ViewFlag").Value) = -1 then	%>
                          			<li><a href='/ShowDetail.asp?ID=<%=(ShowDisciplines.Fields.Item("ID").Value)%>' title='<%=(ShowDisciplines.Fields.Item("NewsName").Value)%>'><%=(ShowDisciplines.Fields.Item("NewsName").Value)%></a></li>
                            <% 	end if
					...
					<div class="cont">
						<ul class='list'>
						  ...
						  ShowList.AbsolutePage =  PageNum	'将记录指针移到第PageNum页的第一行
							...
							%>
                          			<li><a href="/ShowDetail.asp?ID=<%=(ShowList.Fields.Item("ID").Value)%>" title="<%=(ShowList.Fields.Item("NewsName").Value)%>" style=" font-weight:<%=(ShowList.Fields.Item("bold").Value)%>; color:<%=(ShowList.Fields.Item("color").Value)%>"><%=(ShowList.Fields.Item("NewsName").Value)%></a> <span><%=(ShowList.Fields.Item("AddTime").Value)%></span></li>
                            <% 	end if
						...
						<div class="Page">
						<% if PageNum <> 1 then %>
							<a href='/ShowNewsList.asp?SortID=<%=ShowList__MMColParam%>&PageNum=<%=PageNum-1%>'>上一页</a>
						<% end if %>    
							
							<a href="" Class="selected"><%=PageNum%></a>
							
						<% if cstr(PageNum) <> cstr(ShowList.PageCount) then %>
							<a href='/ShowNewsList.asp?SortID=<%=ShowList__MMColParam%>&PageNum=<%=PageNum+1%>'>下一页</a>
						<% end if %>  
