...
ShowDetail.Source = "SELECT AddTime, ClickNumber, Content, ID, NewsName, Origin, SortID, UpdateTime, ViewFlag FROM keyicms_News WHERE ID = " + Replace(ShowDetail__MMColParam, "'", "''") + " ORDER BY AddTime DESC"
...
ShowDisciplines__MMColParam = (ShowDetail.Fields.Item("SortID").Value)
If (Request("MM_EmptyValue") <> "") Then 
		...
		<div class="main">
			<div class="left">
				<div class="box">
					...
					<div class="cont">
						<div class="article">
							<div class='artName'><%=(ShowDetail.Fields.Item("NewsName").Value)%></div>
							<div class='artInfo'>
								新闻来源：<%=(ShowDetail.Fields.Item("Origin").Value)%>&nbsp;&nbsp;
								添加时间：<%=(ShowDetail.Fields.Item("AddTime").Value)%>&nbsp;&nbsp;
								浏览次数：<script language="javascript" src="/Asp/Number.Asp?id=<%=(ShowDetail.Fields.Item("ID").Value)%>&LX=Keyicms_News"></script>
									 <script language="javascript" src="/Asp/Number.Asp?id=<%=(ShowDetail.Fields.Item("ID").Value)%>&action=count&LX=Keyicms_News"></script>
							</div>
