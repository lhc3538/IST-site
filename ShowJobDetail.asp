...
ShowDetail.Source = "SELECT * FROM keyicms_Job WHERE ID = " + Replace(ShowDetail__MMColParam, "'", "''") + ""
...
ShowDisciplines.Source = "SELECT ID, JobName FROM keyicms_Job WHERE ViewFlag = " + Replace(ShowDisciplines__MMColParam, "'", "''") + " ORDER BY AddTime DESC"
...
<div class="main_div3">
	<div class="main">
    	<div class="left">
        	<div class="box">
            	<div class="bar">
                	<div class="tit tit03">详细信息</div>
                </div>
                <div class="cont">
                	<div class="article">
                        <div class='artName'><%=(ShowDetail.Fields.Item("JobName").Value)%></div>
                        <div class='artInfo'>
                            发布时间：<%=(ShowDetail.Fields.Item("AddTime").Value)%>&nbsp;&nbsp;
                            浏览次数：<script language="javascript" src="/Asp/Number.Asp?id=<%=(ShowDetail.Fields.Item("ID").Value)%>&LX=Keyicms_Job"></script>
									 <script language="javascript" src="/Asp/Number.Asp?id=<%=(ShowDetail.Fields.Item("ID").Value)%>&action=count&LX=Keyicms_Job"></script>
                        </div>
                        <div class='artCont'>
                            <table class="recruit">
                            <tr>
                                <td><strong>招聘人数：</strong><%=(ShowDetail.Fields.Item("JobNumber").Value)%></td>
                                <td><strong>薪水待遇：</strong><%=(ShowDetail.Fields.Item("Emolument").Value)%></td>
                            </tr>
                            <tr>
                                <td width="50%"><strong>工作地址：</strong><%=(ShowDetail.Fields.Item("JobAddress").Value)%></td>
                                <td width="50%"><strong>有效时间：</strong><%=(ShowDetail.Fields.Item("StartDate").Value)%> - <%=(ShowDetail.Fields.Item("EndDate").Value)%></td>
                            </tr>
                            </table>
                            <div class="recruit">
                                <%=(ShowDetail.Fields.Item("Requirement").Value)%>
                            </div>
                        </div>
                	</div>
                </div>
		...
		<div class="right">
			<div class="box">
				<div class="bar">
					<div class="tit tit03">最新 就业信息</div>
				</div>
				<div class="cont">
					<ul class="list">
					  <% While ((Repeat1__numRows <> 0) AND (NOT ShowDisciplines.EOF)) %>
								<li><a href='/ShowJobDetail.asp?ID=<%=(ShowDisciplines.Fields.Item("ID").Value)%>' title='<%=(ShowDisciplines.Fields.Item("JobName").Value)%>'><%=(ShowDisciplines.Fields.Item("JobName").Value)%></a></li>
						<%
